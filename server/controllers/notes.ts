...
router.post(
  ...
  async (req: any, res, next) => {
    ...
    if (req.body.tags != [] && req.body.tags != "") {
      var tags = req.body.tags.map((t) => t.label);
      var tagsIds = [];
    }
    try {
      ...
      if (validationErrors) {
        ...
        if (tags) {
          for (const tag of tags) {
            let newTag = await models.Tag.findOrCreate({
              where: { name: tag },
            });
...
router.get(
  ...
  async (req: any, res, next) => {
    ...
    try {
      let notes;
      if (search) {
        notes = await models.Note.searchFilterMyNotes(
          search,
          req.user.id,
          limit,
          offset
...
//find note by id
router.get("/:id", authorize.getUser, async (req: any, res, next) => {
  try {
    ...
    try {
      ...
      if (validationErrors) {
        ...
        if (req.user.id != note.userId) {
          return res
            .status(401)
            .json({ errors: ["Unauthorized to edit this note"] });
        }
        ...
        if (note) {
          ...
          await note.setTags(tagsIds);
          const changedNote = await models.Note.findByPk(req.params.id, {
            include: [
              { model: models.Tag, as: "tags", through: { attributes: [] } },
            ],
          });
...
router.get(
  ...
  async (req: any, res, next) => {
    ...
    try {
      let notes;
      if (search) {
        notes = await models.Note.searchFilterUsersNotes(
          search,
          req.user.id,
          limit,
          offset
  ...
  async (req: any, res, next) => {
    try {
      ...
      if (note && note.userId == req.user.id) {
        ...
        res.status(500).json({
          errors: ["Note not found, or this user is not authorized to delete"],
        });
      }
    } catch (err) {
      next(err);
